...
async def main(message, canary=False):
    try:
        ...
        futures_prices = [x for x in futures_prices if x != 0]
        contract_months = contract_months[:len(futures_prices)]
        index_prices = contract_months[:len(futures_prices)]
        ...
        fig.update_layout(template="plotly_dark",
                          title=f"VIX Calendar Curve for {r_json[1][0].split(' ')[0]}", width=1440)
